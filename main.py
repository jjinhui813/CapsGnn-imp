            ...
            for i in range(FLAGS.epochs):
                ...
                if i == FLAGS.epochs-1:
                    for test_iter in range(10):
                        ...
                        t0 = time()
                        while one_epoch == 0:
                            attri_descriptors, adj_mats, label, reconstructs, one_epoch = gd.data_gen(
                                data_source=gd.graphs_dataset_test, batch_size=FLAGS.batch_size)
                            attri_descriptors = list(zip(*attri_descriptors))
                            feed_dict = dict()
                            for idx in range(len(attri_descriptors)):
                                feed_dict[GraphNet.node_inputs[idx]] = attri_descriptors[idx]
                        ...
                        test_time = time() - t0
                        tf.logging.info(
                            '#########################  TEST\tEpoch: %d : loss : %f, error : %f, time : %.2f sec.  #####################' % (
                            i, loss / step, error / (1.0 * len(gd.graphs_dataset_test)), test_time))
                        loss_error_record.extend(['no:',test_iter,'test : ', str(loss / step), str(error / (1.0 * len(gd.graphs_dataset_test))), '\n'])
