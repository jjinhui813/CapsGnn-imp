        ...
        with graph.as_default():
            with tf.device('/cpu:0'):
                ...
                for layer_idx, C_node_config in enumerate(self.net_structure['node_emb']):
                    ...
                    if layer_idx == 0:
                        Ci_node = len(self.node_attris) + pre_c
                    else:
                        Ci_node = self.net_structure['node_emb'][layer_idx-1] + pre_c
                    Co_node = self.net_structure['node_emb'][layer_idx]
                    name = 'node_embedding_' + str(layer_idx)

                    #保存之前的输入
                    pre_input = node_embeddings
                    pre_c = Ci_node
                    ...
                    #残差连接
                    node_embeddings = tf.concat([pre_input,node_embeddings], axis=2)
