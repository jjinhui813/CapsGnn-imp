...
def capsule_layer(inputs, Ci, Co, in_emb_size, graph_emb_size, iterations, position_emb_size,nodes_indicator, batch_size, coordinate, name):
    ...
    with tf.variable_scope(name) as scope:
        ...
        with tf.variable_scope('routing') as scope:
            num_nodes = tf.reduce_mean(nodes_indicator, axis=1, keep_dims=True)  # (?, #1, 1)
...
def loss_layer(inputs,reconstruct_value,label,lambda_val,batch_size,num_class,reconstruct_num,name,reg_scale):
    ...
    with tf.variable_scope(name) as scope:
        ...
        '''
        with tf.variable_scope('k_means_loss') as scope:
            v_j = tf.reshape(v_j, shape=[batch_size, num_class, input_shape[-1]])
            vectors = tf.zeros(batch_size, num_class + 1)
            class_node=tf.zeros(num_class+1)
            counts = tf.zeros(num_class + 1)
            for i in range(batch_size):
                vectors[i]=v_j[i][label[i]]
                class_node[label[i]] = class_node[label[i]] + v_j[i][label[i]]
                counts[label[i]]=counts[label[i]]+1
            for i in range(num_class):
                class_node[i]=vectors[i]/counts[i]
            for i in range(batch_size):
                vectors[i]=distance(class_node[label[i]],vectors[i])

            loss=tf.reduce_sum(vectors)'''
...
def error_layer(input,label,batch_size,num_class,name):
    ...
    with tf.variable_scope(name) as scope:
        ...
        error = tf.cast(tf.equal(pred,label),dtype=tf.int32)
...
def adj_mats_normalize(batch_adj_mats, node_indicator):
    ...
    with tf.variable_scope('adj_mats_normal') as scope:
        ...
        #添加对角线参数
        w_diag = tf.Variable(initial_value=0, trainable=True, name="diag_variable", dtype=tf.float32)
        temp = tf.eye(tf.shape(r_mat_inv)[1])*-1
        temp = temp[tf.newaxis,:,:]
        temp = tf.tile(temp,[tf.shape(r_mat_inv)[0],1,1])*node_indicator

        temp2 = tf.eye(tf.shape(r_mat_inv)[1])*w_diag
        temp2 = temp2[tf.newaxis,:,:]
        temp2 = tf.tile(temp2, [tf.shape(r_mat_inv)[0],1,1])*node_indicator

        batch_adj_mats = batch_adj_mats -temp + temp2
